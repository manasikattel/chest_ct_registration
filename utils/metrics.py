...
def TRE_measure(inhale_landmarks, exhale_landmarks, patient_number):
    ...
    voxel_size = metadata.loc[f'{patient_number.lower()}'][[
        'vspacing0', 'vspacing1', 'vspacing2'
    ]]
...
def metrics_4_all(folder_experiment_landmarks):
    ...
    inhale_transform_points = [
        i for i in datadir_inhale.rglob("*.txt") if "copd" in str(i)
    ]
    patient = []
    for inhale_transform in inhale_transform_points:
        inhale_point = np.array([
            i for i in pd.read_csv(
                inhale_transform, sep="=|;", header=None, engine='python')
            [6].apply(lambda x: x.strip().split("]")[0].split("[")[1].split(
                " ")[1:4])
        ]).astype(np.int16)
        exhale_point = np.loadtxt(
            datadir_exhale / inhale_transform.parent.stem /
            f"{inhale_transform.parent.stem}_300_eBH_xyz_r1.txt").astype(
                np.int16)
        mean, std = TRE_measure(inhale_point, exhale_point,
                                inhale_transform.parent.stem)
        patient.append({
    ...
    metrics_df.to_csv(
        Path(thispath / f"metrics/metrics_{folder_experiment_landmarks}.csv"))
    print(
        f"TRE computed and results saved as .csv file in metrics/{folder_experiment_landmarks}"
    )
...
@click.option(
    ...
    help=
    "Name of the experiment folder coming from transformix with the transformed points of the inhale image of "
